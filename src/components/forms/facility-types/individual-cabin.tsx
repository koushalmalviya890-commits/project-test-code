...
import { RENTAL_PLANS, AMENITIES, IndividualCabinFields, FacilityFormProps } from '../types/index'
...
const formSchema = z.object({
  ...
  coupons: z.array(z.object({
    ...
    minimumValue: z.number().min(0, 'Minimum value must be 0 or greater'),
...
export function IndividualCabinForm({ onSubmit, onChange, initialData }: FacilityFormProps) {
 // console.log('IndividualCabinForm initialData:', initialData);
 // console.log('IndividualCabinForm address fields:', {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      totalCabins: initialData?.totalCabins || undefined,
      availableCabins: initialData?.availableCabins || undefined,
...
<div className="space-y-4">
  <div className="flex items-center justify-between">
    <div>
      <h3 className="text-lg font-semibold">Promotional Coupons</h3>
      <p className="text-sm text-muted-foreground mt-1">
        Create discount coupons that users can apply during booking
      </p>
    </div>
    <Button
      type="button"
      ...
      onClick={() => {
        ...
        form.setValue('coupons', updatedCoupons);
      }}
    >
      Add Coupon
    </Button>
      ...
      {coupons.map((coupon, index) => (
        <div key={index} className="border rounded-lg p-4 space-y-4 bg-muted/30">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField
              control={form.control}
              name={`coupons.${index}.couponCode` as any}
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        updated[index].couponCode = e.target.value;
                        setCoupons(updated);
                      }}
                    />
                  </FormControl>
            ...
            <FormField
              control={form.control}
              name={`coupons.${index}.discount` as any}
              render={({ field }) => (
                <FormItem>
          ...
          <FormField
            control={form.control}
            name={`coupons.${index}.minimumValue` as any}
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      field.onChange(val);
                      const updated = [...coupons];
                      updated[index].minimumValue = val;
                      setCoupons(updated);
                    }}
                  />
                </FormControl>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField
              control={form.control}
              name={`coupons.${index}.validFrom` as any}
              render={({ field }) => (
                <FormItem>
            ...
            <FormField
              control={form.control}
              name={`coupons.${index}.validTo` as any}
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      ...
                      onChange={(e) => {
                        field.onChange(e);
                        const updated = [...coupons];
                        updated[index].validTo = e.target.value;
                        setCoupons(updated);
                      }}
                    />
                  </FormControl>
