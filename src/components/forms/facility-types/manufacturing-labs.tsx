...
const formSchema = z.object(
  {
    ...
    // pricingModel: z.enum(["rental", "booking"]).default("rental"),
...
export function ManufacturingLabsForm({
  onSubmit,
  onChange,
  initialData,
}: FacilityFormProps) {
 // console.log("ManufacturingLabsForm initialData:", initialData);
 // console.log("ManufacturingLabsForm address fields:", {
  ...
  // const { watch } = form;  // Get watch from form instance
  ...
  const addEquipment = () => {
    setEquipment([
      ...equipment,
      { labName: "", equipmentName: "", capacityAndMake: "" },
    ]);
  ...
  const handleFormSubmit = async (values: FormValues) => {
    try {
     ...
     // console.log("Form values:", values);
      ...
      const validEquipment = values.equipment.filter(
        (eq) =>
          eq.labName.trim() !== "" &&
      ...
      if (validEquipment.length === 0) {
       // console.log("Validation failed: No valid equipment data");
        form.setError("root", {
          type: "manual",
          message:
            "Please add at least one equipment with lab name, equipment name, and capacity",
        });
        return;
      }
      ...
      const submissionData = {
        ...
        facilityType: "manufacturing-labs",
      ...
      // Cast the entire submission data to any to bypass type checking
          ...
          {sectorTags.map((sector) => {
            ...
            return (
              <div
                ...
                className={`border rounded-lg px-3 py-2 text-sm cursor-pointer transition-colors select-none ${isSelected
        ...
        >
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>
                  If available, kindly attach Video Link for your Facility
                </FormLabel>
                <FormControl>
                  <Input
                    type="url"
          ...
          {/* Booking Plan Categories Section */}


          {/* type of booking for equipment */}
