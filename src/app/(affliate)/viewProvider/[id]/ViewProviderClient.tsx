...
export default function ViewProviderClient({
  ...
  useEffect(() => {
    const fetchServiceProvider = async () => {
      try {
        setLoading(true);
        const response = await fetch(`${baseUrl}/api/service-provider/${providerId}`);
        if (!response.ok) {
        ...
        if (provider.address) {
          ...
          const mapResponse = await fetch(`${baseUrl}/api/maps?query=${encodeURIComponent(fullAddress)}`);
          if (mapResponse.ok) {
  ...
  useEffect(() => {
    const fetchFacilities = async () => {
      try {
        ...
        const response = await fetch(`${baseUrl}/api/service-provider/${providerId}/facilities?page=${pagination.page}&limit=${pagination.limit}`);
