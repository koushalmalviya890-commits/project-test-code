...
//       db.collection('AffiliateLinkUsers').findOne({ mailId: booking.affiliateUserEmail }),
...
//         // Use mailId instead of email for the startup
//         recipientEmail = startup.mailId || startup.email || '';
//         if (!recipientEmail) {
//           console.error(`No mailId or email found for startup ${startup._id}`);
//         }
//       } else {
...
//               <p>Hi ${startup.contactName || 'there'},</p>
...
export async function POST(req: NextRequest) {
  try {
    // Get the raw request body for signature verification
    const data = await req.json();
    const { bookingId, automated, recipientEmail: automatedRecipient, forceSend } = data;

    // Check for automation header for internal system requests
    const automationSecret = req.headers.get('X-Invoice-Automation');
    const isAutomatedRequest =
      automationSecret === process.env.EMAIL_WEBHOOK_SECRET && automated === true;
    ...
    if (mongoose.connection.readyState !== 1) {
      await mongoose.connect(process.env.MONGODB_URI!); // Ensure MONGODB_URI is set
    }
    ...
    // Check for duplicate emails - prevent sending multiple emails in a short time frame
    if (booking.invoiceEmailHistory && booking.invoiceEmailHistory.length > 0 && !forceSend) {
      interface EmailHistoryItem {
        sentTo: string;
        sentAt: Date | string;
        sentBy: string;
    ...
    // Look up related data
    const [startup, facility] = await Promise.all([
      User.findOne({ email: booking.affiliateUserEmail }), // Use User model instead of AffiliateLinkUsers
      db.collection('Facilities').findOne({ _id: booking.facilityId }),
    ]);
    ...
    if (isAutomatedRequest && automatedRecipient) {
      ...
      if (booking.startupId.toString() === userId) {
        ...
        if (!recipientEmail) {
          console.error(`No email found for startup ${startup._id}`);
        }
      } else {
        const serviceProvider = await db.collection('Service Provider').findOne({
          userId: booking.incubatorId,
    ...
    try {
      await sendInvoiceEmail(
        recipientEmail,
        startup.name || 'there', // Use name from User model
        facility.details?.name || 'N/A',
        facilityLocation,
        bookingDates,
        facility.facilityType || 'N/A',
